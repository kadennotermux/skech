...
const daftar1 = `Hi ${pushname} ${ucapanWaktu} \n\nBefore Using the Bot Verify First `
       const daftar2 = '```Click the button below to verify please or type #Verify\n🪀ˢᵏᵉᵗᶜʰʸ🪀```'
       const daftar3 = [
          {
            buttonId: `verify`,
            buttonText: {
...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Xeon Bot Inc.`, "footerText": "*_🪀ˢᵏᵉᵗᶜʰʸ🪀_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`🪀ˢᵏᵉᵗᶜʰʸ🪀`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
...
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Xeon`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/254792893310?text=hello`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\n🪀ˢᵏᵉᵗᶜʰʸ🪀. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
*Thank you for registering in the 🪀ˢᵏᵉᵗᶜʰʸ🪀 Database*
...
*「 🪀ˢᵏᵉᵗᶜʰʸ🪀 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              ...
              buttonsMessage = {footerText:'SketchyBot', imageMessage: imageMsg,
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered 🪀ˢᵏᵉᵗᶜʰʸ🪀*`)
		}, 2000)
        break
...
㋛${prefix}𝙩𝙬𝙞𝙩𝙩𝙚𝙧 <𝙪𝙧𝙡>
`
sendButMessage(from, menu, `🪀ˢᵏᵉᵗᶜʰʸ🪀`, [
          {
            buttonId: `command`,
            buttonText: {
...
case 'script':
		case 'sc':
		case 'sourcecode':
		xeon.sendMessage(from, { text: "https://github.com/GingerBreadSketchy", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
     ...
     break
        ...
        case "broadcast":  
                   if (!isOwner && !mek.key.fromMe) return reply('only owner can use this feature')    
					...
					buff = await xeon.downloadMediaMessage(encmedia)
					for (let _ of anu) {
						xeon.sendMessage(_.jid, buff, image, {quoted:fakestatus ,caption: `「  *🪀ˢᵏᵉᵗᶜʰʸ🪀 BROADCAST* 」\n\n${ini_bc}`})
					}
					reply('```Success Broadcast```')
					} else {
					for (let _ of anu) {
					xeon.sendMessage(_.jid, `*🪀ˢᵏᵉᵗᶜʰʸ🪀 BROADCAST*\n${ini_bc}`, text, { contextInfo: { mentionedJid: [sender],"forwardingScore":999,"isForwarded":true},sendEphemeral: true })
					}
					reply('```Success Broadcast```')
					}
					break
...
case 'playm2': case 'ytmp3':
									...
									try {
										...
										.then((res) => {
											...
											.then(async (a) => {
												if (Number(filesize) >= 100000) return sendMediaURL(from, thumb, `*🎧SKETCHY MUSIC PLAYER🎧*\n\n*Title* : ${title}\n*Ext* : MP3\n*Filesize* : ${filesizeF}\n*Link* : ${a.data}\n\n_For the duration of more than the limit is presented in the form of a link_`)
												const captions = `🎧 *🎧SKETCHY MUSIC PLAYER🎧*\n\n*Title* : ${title}\n*Ext* : MP3\n*Size* : ${filesizeF}\n*Link* : ${a.data}\n\n_Please wait for the media file to be sent it may take a few minutes_`
...
if (!isGroup) return reply(`*Only group*`)
             ...
             xeon.sendMessage(from, '*🪀🪀🪀ˢᵏᵉᵗᶜʰʸ Online List🪀:*\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
...
buttons = [{buttonId: `writelist`,buttonText:{displayText: `Write List🔖`},type:1}]
              ...
              buttonsMessage = {footerText:'🪀ˢᵏᵉᵗᶜʰʸ🪀', imageMessage: imageMsg,
...
sendButMessage(from, tiyo, `🪀ˢᵏᵉᵗᶜʰʸ🪀`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 🪀ˢᵏᵉᵗᶜʰʸ🪀 BROADCASTING 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu) {
xeon.sendMessage(_.jid, 
			{"contentText": `*🪀🪀skeTcHyB̊oT Broadcast🪀*\n\n${body.slice(4)}`,
			"footerText": '🪀ˢᵏᵉᵗᶜʰʸ🪀',
			"buttons": [
          ...
          ytv(mulaikah).then((res) => {
            ...
            axios
              ...
              .then(async (a) => {
                ...
                const captions = `*📛SKETCHY VIDEO PLAY📛*\n\n*Title* : ${title}\n*Ext* : MP4\n*Size* : ${filesizeF}\n*Link* : ${a.data}\n\n_Please wait for the media file to be sent it may take a few minutes_`;
                sendMediaURL(from, thumb, captions);
                await sendMediaURL(from, dl_link).catch(() => reply("error"));
              });
          });
